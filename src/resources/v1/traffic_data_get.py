...
class Get_t2_time_range(Resource):
    ...
    def get(self, dataclass, authority, oid, sdate, edate):
        ...
        命令格式： /v1/traffic_data/class/{dataclass}/authority/{authority}/id/{id}/date/{sdate}/to/{edate}/standard/MOTC_traffic_v2/method/time_range?format={format}
        ---
        tags:
          - Traffic Data Query API (交通資料查詢API)
        parameters:
        ...
        if ('Live' in dataclass) or dataclass == 'News':
            # 動態資料查詢管道指令
            pipeline = "{'$match':{'$and':[{'" + id_name + "':'" + oid + "'},{'DataCollectTime':{'$gte':{'$date':'" + sdate + "'},'$lte':{'$date':'" + edate + "'}}}]}}"
        else:
            # 靜態資料查詢管道指令
            pipeline = "{'$match':{'$and':[{'" + id_name + "':'" + oid + "'},{'UpdateTime':{'$gte':{'$date':'" + sdate + "'},'$lte':{'$date':'" + edate + "'}}}]}}"
        df = spark.read.format('mongo').option('uri', mongo_url_db).option('pipeline', pipeline).load()
        json_data_list = df.toJSON().collect()
        json_data = []
        for values in json_data_list:
